...
class UserView extends Croquet.View {
  constructor(model) {
    ...
    if (this.isMyUser) {
      ...
      cameraRig.appendChild(this.rightHand);
      ...
      this.publishCameraMatrix = AFRAME.utils.throttle(
        () => {
          const hasCameraMovedSinceLastModelMatrixUpdate = this.camera.matrixWorld.elements.every(
      ...
      this.publishLeftHandMatrix = AFRAME.utils.throttle(
        () => {
          if (
            ...
            !this.leftHand.object3D.matrixWorld
          )
            return;

          const moved = this.leftHand.object3D.matrixWorld.elements.every(
            (value, index) =>
              value === this.model.leftHandMatrix.elements[index]
          );
          if (!moved) {
            this.publish(
              this.viewId,
              "set-left-hand-matrix",
              this.leftHand.object3D.matrixWorld
            );
          }
        },
      ...
      this.publishRightHandMatrix = AFRAME.utils.throttle(
        () => {
          if (
            ...
            !this.rightHand.object3D.matrixWorld
          )
            return;

          const moved = this.rightHand.object3D.matrixWorld.elements.every(
            (value, index) =>
              value === this.model.rightHandMatrix.elements[index]
          );
          if (!moved) {
            this.publish(
              this.viewId,
              "set-right-hand-matrix",
              this.rightHand.object3D.matrixWorld
            );
          }
        },
